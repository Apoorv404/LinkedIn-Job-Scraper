...
# Get ROLE and LOCATION from frontend, or use defaults
if len(sys.argv) > 2:
    ROLE = sys.argv[1]
    LOCATION = sys.argv[2]
else:
...
jobs_title_tag = soup.select(selector="a strong")
jobs_company_tag = soup.find_all(class_="artdeco-entity-lockup__subtitle ember-view")
jobs_location_tag = soup.find_all(class_="job-card-container__metadata-wrapper")
...
if len(job_titles) == len(companies) == len(job_locations):
